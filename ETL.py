...
def etl_weather_data():
    ...
    names_of_city = ["Mumbai", "Delhi", "Bangalore", "Hyderabad", "Chennai", "Kolkata", "Pune", "Ahmedabad", "Surat", "Jaipur"]
...
with DAG('weather_dag',
        ...
        task_id= 'tsk-extract-transform-weather-data',
        python_callable=etl_weather_data
        )

        load_to_s3 = BashOperator(
            task_id = 'tsk_load_to_s3',
            bash_command = 'aws s3 mv {{ ti.xcom_pull("tsk-extract-transform-weather-data")}} s3://airflow-slack-bucket',
        )

        slack_notification = SlackWebhookOperator(
            task_id = "tsk_slack_notification",
            slack_webhook_conn_id = "slack_conn_id",
            message = "Hey team! The data is ready in the AWS Airflow-Slack bucket now.",
